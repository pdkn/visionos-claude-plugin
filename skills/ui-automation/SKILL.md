...
- Switch to `debugging-triage` when the question is "did the app emit this
  event?" - AXe inspects the UI surface, not the unified-logging stream.
- Switch to `realitykit` for entity-level manipulation test hooks when a test
  needs to assert spatial gesture state.
...
- explicit routing back to `build-run-debug`, `test-triage`, or
  `debugging-triage` for the next step
