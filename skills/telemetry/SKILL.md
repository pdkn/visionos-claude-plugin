...
Use this skill to add lightweight app instrumentation that helps debug behaviour without turning a spatial app into a logging landfill. Prefer Apple's unified logging APIs and verify the events after a build/run loop (via `build-run-debug` and the post-build log capture hook).
...
1. Identify the behaviour that needs observability.
2. Add the smallest useful instrumentation.
3. Build and run the app via `build-run-debug`.
...
- Switch to `build-run-debug` when build, install, launch, simulator, or LLDB debugging becomes the main blocker.
- Switch to `debugging-triage` when telemetry reveals a runtime failure that needs systematic root-cause analysis.
- Switch to `spatial-architecture` when telemetry shows a structural ownership problem in scene composition, state placement, or lifecycle boundaries.
- Switch to `signing-entitlements` when telemetry or launch output indicates capability, entitlement, privacy-key, or sandbox-denial configuration issues.
- Switch to `perf-90hz` when telemetry shows timing regressions or frame budget pressure.
- Return to `telemetry` after fixes to confirm the corrected lifecycle path with a minimal high-signal log set.
...
- If the debugging task is mostly about crash/backtrace analysis rather than action telemetry, switch to `build-run-debug` or `debugging-triage`.
- If telemetry indicates configuration denial instead of logic failure, route to `signing-entitlements` instead of expanding logs indefinitely.

## Output Expectations

Provide:
- the behaviour instrumented
